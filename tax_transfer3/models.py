...
# set rd1a and rd1b among rd1, .., rd7
    ...
    def set_round_group(self):
        ...
        rd1a_period3 = random.randint(1, Constants.num_rounds)
        rd1b_period3 = random.randint(1, Constants.num_rounds)

        for round in range(0, Constants.num_rounds):
            for p in self.get_players():
                if round == 0:
                    p.participant.vars['decgroup_period3'] = []
                    p.participant.vars['identity_period3'] = []
            ...
            groups400 = []
            groups400.append(
                [p for p in self.get_players() if p.participant.vars['pt_group_id'] == 1 and p.income3 == 400])
            groups400.append(
                [p for p in self.get_players() if p.participant.vars['pt_group_id'] == 2 and p.income3 == 400])
            groups400.append(
                [p for p in self.get_players() if p.participant.vars['pt_group_id'] == 3 and p.income3 == 400])

            groups200 = []
            groups200.append(
                [p for p in self.get_players() if p.participant.vars['pt_group_id'] == 1 and p.income3 == 200])
            groups200.append(
                [p for p in self.get_players() if p.participant.vars['pt_group_id'] == 2 and p.income3 == 200])
            groups200.append(
                [p for p in self.get_players() if p.participant.vars['pt_group_id'] == 3 and p.income3 == 200])
            ...
            for pt_group in range(0, 3):
                ...
                groups800_600[pt_group].append(
                    [p for p in self.get_players() if
                     p.income3 == 800 and p.participant.vars['pt_group_id'] == pt_group + 1])
                groups800_600[pt_group].append(
                    [p for p in self.get_players() if
                     p.income3 == 600 and p.participant.vars['pt_group_id'] == pt_group + 1])
            ...
            for dg_index in range(0, len(final_groups)):
                ...
                identity = 0
                difs.append(cur_group[2].participant.vars['nt3_points'] - cur_group[3].participant.vars['nt3_points'])
                ...
                for member in cur_group:
                    member.participant.vars['decgroup_period3'].append(dg_index + 1)
                    member.participant.vars['identity_period3'].append(identity)
                if round == 7:
                    for member in cur_group:
                        print('member: ', member.participant.vars['decgroup_period3'],
                              member.participant.vars['identity_period3'])
            for dif in difs:
    ...
    def set_rg_tokens(self):
        ...
        for p in self.get_players():
            ...
            p.rg8_tokens = p.participant.vars['rg_tokens_period3'][7]
...
class Player(BasePlayer):
    ...
    def set_info(self, round_var):
        ...
        self.identity = self.participant.vars['identity_period3'][round_var-1]
...
# Compare dif of this round: token 400's raw pts - token 200's raw pts > dif median?
    ...
    def find_group_members_info(self, round_var):
        ...
        result[int((self.income3 - 200) / 200)] = [self.pt_group_id,self.income3,self.participant.vars['nt3_points']]
    ...
    def set_competition(self):
        self.nt_dif_median = self.participant.vars['nt3_dif_median']
