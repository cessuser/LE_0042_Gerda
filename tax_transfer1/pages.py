...
class Period1(Page):
    ...
    def vars_for_template(self):
        if self.round_number == 1:
            self.player.participant.vars['competition_period1'] = []
            self.player.participant.vars['dv_period1'] = []
            self.player.participant.vars['rg_tokens_period1'] = []
        ...
        lst = [200,400,600,800]
        lst.remove(self.player.income1)
        random.shuffle(lst)
    ...
    def before_next_page(self):
        self.player.participant.vars['dv_period1'].append(self.player.dv)
